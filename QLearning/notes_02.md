...
$R(z)$는 종료 상태까지 가야 알 수 있다.(MC) 
따라서 다음 state(이웃 상태)의 가치 $s'$와 현 상태의 보상 $r$으로 대치하여 식을 유도한다.(벨만 방정식의 항으로 대치)

$v^{(new)}(s) =v^{(old)}(s) + \frac{1}{n+1}(R(z)-v^{(old)}(s))$

$v(s) = v(s) + \Rho((r+\gamma \cdot v(s')) - v(S))$

$r+\gamma \cdot v(s')$를 학습 알고리즘이 맞혀야 히는 정답으로 간주하고 $v(s)$를 알고리즘의 출력, 예측 값이라고 간주한다. 새로 발생한 정보를 가지고 오차를 줄이는 방향으로 가치 함수를 개선하는 역할을 한다고 볼 수 있다. 여기서 $\Rho$는 학습률(learning rate)로서 갱신하는 양을 조절하는 하이퍼 파라미터다.

$r+\rho v(s') - v(s)$는 시간차 오차라고 부른다. 학습과정을 개념화한 식은 다음과 같다. 
> Esitimate_{new} = Estimate_{old} _ \Rho(Target - Esitimate_{old})$

강화 학습의 시간차 학습 알고리즘은 에피소드를 생성하며 발생하는 순간 정보를 가지고 가치 함수 또는 정책을 개선한다. 
...
최적 정책을 찾는 원리, On-policy
> 초기 정책 π를 가지고 시작하여 행동 가치함수를 추정하고, 추정한 가치 힘수로 정책을 개선한다. 이렇게 추정과 개선 사이클을 수렴할 때까지 반복한다. 가치 함수를 계산할 때는 항상 최신 정책이 생성한 에피소드 데이터에 의존한다. 단, 온 정책은 현재 정책에 의존하기 때문에 탐사에 치우칠 위험이 있다. 
